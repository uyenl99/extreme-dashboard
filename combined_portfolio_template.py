...
def build_chart(daily, strategy_name="Dual Momentum"):
    fig = go.Figure()
    fig.add_trace(
        go.Scatter(
            x=daily["Date"],
...
def render_page(summary, daily, allocations, monthly, alert, partial=None, *, results_only=False, title="MoMoEtf1", description=None, chart_html=None, monthly_html=None, chart_description=None):
    chart_name = title if results_only else "Dual Momentum"
    chart_html = chart_html if chart_html is not None else build_chart(daily, chart_name)
    monthly_html = monthly_html if monthly_html is not None else build_monthly_table(monthly, partial)
    chart_description = chart_description or f"{chart_name} compared with an equal-starting-equity SPY benchmark."
    description = description or "Systematic ETF allocation model that adjusts monthly across major market exposures using proprietary trend and risk-management signals. Subscribers receive current model allocations and update alerts."
    faq_html = "" if results_only else render_faq()
    eyebrow = "Backtested combined portfolio" if results_only else "Backtested ETF allocation model"
...
<title>{html.escape(title)} Backtest - Extreme Trading Inc.</title>
<style>
...
<script async src="https://www.googletagmanager.com/gtag/js?id=G-H3TCFHTVRV"></script>
<script>
...
<section class="panel"><h2>Equity Curve</h2><p class="subtle">{html.escape(chart_description)}</p><div class="chart">{chart_html}</div></section>
<section class="panel"><h2>Monthly Returns</h2>{monthly_html}</section>
{member_sections}
...
import re
def replace_card(text: str, title: str, cagr: float, sharpe: float, drawdown: float,
                 spy_drawdown: float, spy_cagr: float | None = None,
                 spy_sharpe: float | None = None) -> str:
    pattern = re.compile(
        rf"(<h2>{re.escape(title)}</h2>.*?<p class=\"(?:card|home)-stats\">).*?(</p>)",
        re.DOTALL,
    )
    def replacement(match: re.Match[str]) -> str:
        if spy_cagr is not None and spy_sharpe is not None:
            stats = (
                (cagr, f'{cagr:.2%} Strategy CAGR'),
                (sharpe, f'{sharpe:.2f} Strategy Sharpe'),
                (drawdown, f'{drawdown:.2%} Strategy Max Drawdown'),
                (spy_drawdown, f'{spy_drawdown:.2%} SPY Max Drawdown'),
            )
            badges = ''.join(f'<span class="{"negative" if value < 0 else "positive"}">{label}</span>\n'
                             for value, label in stats)
            return f'{match.group(1)}\n{badges}{match.group(2)}'
        return (
            f'{match.group(1)}\n<span class="positive">{cagr:.2%} Backtest CAGR</span>\n'
            f'<span class="positive">{sharpe:.2f} Sharpe Ratio</span>\n'
            f'<span class="negative">{drawdown:.2%} Max Drawdown</span>\n'
            f'<span class="negative">{spy_drawdown:.2%} SPY Max Drawdown</span>\n'
            f'{match.group(2)}'
        )

    updated, count = pattern.subn(replacement, text, count=1)
    if count != 1:
        raise RuntimeError(f"Could not locate the {title} card")
    return updated
