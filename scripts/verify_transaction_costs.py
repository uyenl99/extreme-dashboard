...
p=R/'HAA/results'; e=pd.read_csv(p/'exact_etfs_daily_equity.csv',index_col=0,parse_dates=True); t=pd.read_csv(p/'exact_etfs_HAA_net_5bp_trades.csv',parse_dates=['date']); fees=pd.Series(1-.0005*t.traded_notional.values,index=t.date).reindex(e.index,fill_value=1).cumprod(); equal(e['HAA net 5bp']/e['HAA gross'],fees,'HAA fees')
