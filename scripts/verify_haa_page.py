...
assert sum(cell!='—' for row in monthly[1:] for cell in row[1:13])==len(r)==129
...
for filename in ['strategies.html','members.html']:
    ...
    assert '12.0% Backtest CAGR' in s and '15.9% Backtest CAGR' not in s
...
snapshot=json.loads((source/'current_snapshot.json').read_text()); assert snapshot['as_of']=='2026-09-04'
assert '2026-09-04' in member and '0.08%' in member
...
print('PASS: card metrics, 129 monthly returns, annual compounding, charts, allocation dates, snapshot, and public/member boundaries')
