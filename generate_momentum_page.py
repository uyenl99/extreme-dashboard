...
def load_results(source):
    ...
    return summary.iloc[0], daily, allocations, monthly
...
def render_page(summary, daily, allocations, monthly):
...
<title>Dual Momentum Backtest - Extreme Trading Inc.</title>
<style>
...
def main():
    args = parse_args()
    summary, daily, allocations, monthly = load_results(args.source)
    args.output.write_text(
        render_page(summary, daily, allocations, monthly),
        encoding="utf-8",
    )
