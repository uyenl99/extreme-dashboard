...
def faq(audience):
    items=[('What is HAA?','Hybrid Asset Allocation was developed by Wouter Keller and JW Keuning. This implementation follows the corrected rules described by Allocate Smartly; it is not a proprietary strategy or an affiliation with its authors.'),('How does it allocate?','Each month, average the trailing 1, 3, 6 and 12-month total returns. Positive TIP momentum allows four 25% slots from SPY, IWM, EFA, EEM, VNQ, PDBC, IEF and TLT. Nonpositive slots move to whichever of IEF or BIL has stronger momentum. If TIP momentum is nonpositive, the entire portfolio moves to that defensive choice.'),('Why is the CAGR different from Allocate Smartly’s 15.9%?','Their published 15.9% covers 1971–January 2023, including reconstructed asset history. This actual-ETF sample starts in December 2015. It does not reproduce the earlier simulation.'),('What do Sharpe and drawdown mean here?','Sharpe uses daily returns, 252 trading sessions per year and a zero risk-free rate, matching ETF1. Maximum drawdown includes daily losses, unlike the month-end drawdown in Allocate Smartly’s article.'),('When are these results updated?','The performance statistics cover complete months only. Always use the dates printed on this page. The current allocation snapshot is separately dated; automatic HAA updates and email delivery have not been enabled.')]
